...
6. **沙箱/受限环境兜底**：若在沙箱或受限网络环境启动后，用户浏览器无法访问 `localhost:8618`（常见症状：服务日志显示已启动，但访问无响应，或跨进程 HTTP 被网络策略切断），不要反复重启服务；直接把手动启动命令发给用户在本机终端运行（见第 2 条），并继续以静态 HTML（`-简历-ATS.html` / `-简历-现代.html`）作为主交付物。
